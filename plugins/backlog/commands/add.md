...
Normally `/backlog:add` never reaches you at all: a `UserPromptExpansion` hook
...
- The hook errored before it could write. Reproduce it directly:
  ...
  ```
  echo '{"command_name":"backlog:add","command_args":"test","cwd":"'"$PWD"'"}' \
    | python3 "${CLAUDE_PLUGIN_ROOT}/hooks/backlog_hook.py" --project "$PWD"
  ```

  It should print JSON containing `"decision": "block"`.

Offer to do it for them with `${CLAUDE_PLUGIN_ROOT}/bin/backlog add ...`,
