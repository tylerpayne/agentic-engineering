---
description: Park a note for Claude without spending a turn
...
Normally `/later:push` never reaches you at all: a `UserPromptExpansion` hook
...
- The hook errored before it could write. Reproduce it directly:
  ...
  ```
  echo '{"command_name":"later:push","command_args":"test","cwd":"'"$PWD"'"}' \
    | python3 "${CLAUDE_PLUGIN_ROOT}/hooks/later_hook.py" --project "$PWD"
  ```

  It should print JSON containing `"decision": "block"`.

Offer to do it for them with `${CLAUDE_PLUGIN_ROOT}/bin/later push <message>`, quoting
